# 컴퓨터 비전 이미지 분류 실습 (MNIST, CIFAR-10 CNN)

컴퓨터 비전 수업의 5번째 실습 과제 저장소입니다.  
이번 과제는 MNIST 손글씨 숫자 이미지를 이용한 간단한 신경망(Dense) 분류기와, CIFAR-10 데이터셋을 활용한 합성곱 신경망(CNN) 분류기를 직접 구현하고 성능을 평가하는 것을 목표로 합니다.  
강의에서 배운 분류(Classification)의 개념, 데이터 전처리(정규화), CNN 구조(Conv2D, MaxPooling2D, Flatten, Dense), 그리고 모델의 정확도 평가를 그대로 반영하여 정리했습니다.
...
|---|---|
| Python | 3.9+ |
| 주요 라이브러리 | `tensorflow`, `numpy`, `matplotlib`, `Pillow` |
| 입력 이미지 폴더 | `images/` |
| 결과 이미지 폴더 | `result_images/` |
...
프로젝트 루트(`computer_vison/5/`)에서 터미널을 열고 다음 명령어를 실행합니다.
...
> 참고: 첫 실행 시 MNIST, CIFAR-10 데이터셋이 자동으로 다운로드됩니다. CIFAR-10은 약 170MB이므로 네트워크 환경에 따라 시간이 소요될 수 있습니다.

---

## 실습 01 — 간단한 이미지 분류기 구현 (MNIST)

### 1. 과제에 대한 설명
손글씨 숫자 이미지(MNIST 데이터셋, 28x28 흑백)를 이용하여 간단한 이미지 분류기를 구현합니다.
- MNIST는 0~9까지의 손글씨 숫자 70,000장(훈련 60,000 + 테스트 10,000)으로 구성된 대표적인 이미지 분류 데이터셋입니다.
- `Sequential` 모델과 `Dense` 레이어를 활용하여 간단한 신경망을 구성합니다.
- 데이터를 훈련 세트와 테스트 세트로 분할하고, 모델을 훈련시킨 뒤 정확도를 평가합니다.
- 픽셀 값을 0~1 범위로 정규화하여 학습 수렴 속도를 높였습니다.

### 2. 핵심 코드 설명
```python
# [핵심] MNIST 데이터셋을 자동으로 다운로드하고 훈련/테스트 세트로 분할합니다.
(x_train, y_train), (x_test, y_test) = mnist.load_data()

# [핵심] 픽셀 값을 0~1 범위로 정규화하여 모델의 수렴을 빠르게 합니다.
x_train = x_train.astype("float32") / 255.0

# [핵심] Sequential 모델에 Flatten → Dense(128) → Dense(64) → Dense(10) 레이어를 쌓습니다.
# Flatten은 28x28 이미지를 784차원 벡터로 변환하고, 마지막 Softmax는 10개 클래스의 확률을 출력합니다.
model = Sequential([
...
## 실습 02 — CIFAR-10 데이터셋을 활용한 CNN 모델 구축
...
*(dog.jpg 예측 결과 — 원본 이미지와 10개 클래스별 예측 확률을 함께 보여줍니다.)*
![cifar10_dog_prediction](result_images/02_cifar10_dog_prediction.png)
